    ...
    <div>
      ...
      <div class="grid grid-cols-3 gap-3">
        <div 
          ...
          :class="{ 'opacity-50': isLoading, 'ring-2 ring-primary': selectedIndex === index }"
          @click="() => !isLoading && selectDiceBearAvatar(avatar.url, index)"
        >
          <img :src="avatar.url" :alt="`Avatar option ${index + 1}`" class="w-full h-full object-cover" />
          <div 
...
function generateAvatars() {
  const styles = [
    'avataaars', 'pixel-art', 'bottts', 
    'lorelei', 'personas', 'micah'
  ];
